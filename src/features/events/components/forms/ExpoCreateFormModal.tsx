...
import { createExpoForm } from '../../api/expoFormApi';
import type { ExpoQuestion } from '../../types/expoForm.types';
import { ExpoQuestionCard } from './ExpoQuestionCard';

interface ExpoCreateFormModalProps {
  expoId: string;
  onClose: () => void;
  onSuccess: () => void;
}

export function ExpoCreateFormModal({ expoId, onClose, onSuccess }: ExpoCreateFormModalProps) {
  const [questions, setQuestions] = useState<ExpoQuestion[]>([
    { question_no: 1, question_type: 'text', question_text: '', choices: [], required: false },
  ]);
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh] overflow-hidden"
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-gray-900">สร้างแบบสอบถาม</h2>
                <p className="text-xs text-gray-400">เพิ่มคำถามสำหรับรับ feedback จากผู้เข้าร่วมงาน</p>
              </div>
            </div>
          ...
          <div className="px-6 py-[18px] border-t border-gray-100 flex gap-3 flex-shrink-0">
            ...
            <button onClick={handleSubmit} disabled={isSubmitting || questions.length === 0}
              ...
              style={{ background: 'linear-gradient(135deg,#3674B5,#498AC3)' }}>
              {isSubmitting
                ? <><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"><circle className="opacity-25" cx="12" cy="12" r="10"/><path className="opacity-75" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>กำลังสร้าง...</>
                : <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>บันทึกฉบับร่าง</>
              }
            </button>
          </div>
        </div>
